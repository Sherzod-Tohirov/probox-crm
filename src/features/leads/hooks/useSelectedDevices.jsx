...
export const useSelectedDevices = ({
  ...
  const getActualFirstPayment = useCallback(
    (device) => {
  ...
  const handleRentPeriodChange = useCallback(
    (deviceId, value) => {
      setSelectedDevices((prev) => {
        const updated = prev.map((device) => {
          ...
          const newPeriod =
            Number(value) || Number(device.rentPeriod) || DEFAULT_RENT_PERIOD;
          ...
          return {
            ...
            firstPayment:
              newFirstPayment !== null && newFirstPayment !== undefined
                ? newFirstPayment
                : '',
  ...
  const handleFirstPaymentBlur = useCallback(
    (deviceId) => {
      setSelectedDevices((prev) =>
        prev.map((device) => {
          if (device.id !== deviceId) return device;
          ...
          if (
            ...
            autoCalculatedFirstPayment =
              paymentDetails.calculatedFirstPayment || 0;
          }
          ...
          if (
            ...
            const minAllowedFirstPaymentRaw = Math.round(
              autoCalculatedFirstPayment * 0.9
            );
            // 1000 ga yaxlitlash
            const minAllowedFirstPayment =
              Math.floor(minAllowedFirstPaymentRaw / 1000) * 1000;
          ...
          const roundedFirstPayment =
            Math.floor(currentFirstPayment / 1000) * 1000;
  ...
  const selectedDeviceData = useMemo(() => {
    ...
    return filteredDevices.map((device) => ({
      ...
      totalPayment: (() => {
        ...
        if (
          (isRentPeriodDisabled || isFirstPaymentDisabled) &&
          calculationTypeFilter === 'firstPayment'
        ) {
          return formatCurrencyUZS(0);
        }
  ...
  const totalGrandTotal = useMemo(
    () =>
      selectedDeviceData.reduce((acc, device) => {
        ...
        const originalDevice = selectedDevices.find(
