...
export default function LeadsByCallCenterSection({ data = {}, isLoading = false }) {
  ...
  const totals = stageGroups.reduce((acc, group) => {
    acc[group.key] = centerRows.reduce((sum, row) => sum + Number(row[group.key] || 0), 0);
    return acc;
  }, {});
      ...
      <Row>
        <Col fullWidth>
          {isLoading || isExecutorsLoading ? (
            ...
            <div className="overflow-auto w-full" style={{ maxHeight: '520px' }}>
                  ...
                  {centerRows.map((row, index) => (
                    <TableRow key={`${row.name}-${index}`}>
                      ...
                      {stageGroups.flatMap((group) => {
                        ...
                        const percent = row.leads > 0 ? (value / row.leads) * 100 : 0;
                        return [
