export function calculateLeadLimit(lead, pult, options) {
  ...
  if (options.specialRegions && Array.isArray(options.specialRegions)) {
    if (salary <= 0) {
      if (options.specialRegions.includes(lead?.region)) {
        monthly *= 0.5;
      }
    }
  }
